// ═══════════════════════════════════════════════════════════════════════════
// icons.js — SVG icons and frog animation
//
//   Reference data     → references.js
//   EmailJS config     → config.js
//   Signature system   → signatures.js
//   Wizard engine      → resources.js
// ═══════════════════════════════════════════════════════════════════════════


// ── Mini Frog Animation ──────────────────────────────────────────────────

var MINI_FROG = '<svg width="36" height="36" viewBox="0 0 36 36" fill="none" style="animation:ld-bob 2s ease-in-out infinite">'
  + '<style>@keyframes ld-bob{0%,100%{transform:translateY(0)}50%{transform:translateY(-3px)}}'
  + '@keyframes ld-blink2{0%,48%,52%,100%{opacity:1}50%{opacity:0}}</style>'
  + '<ellipse cx="18" cy="24" rx="10" ry="7" fill="#9DC47A" opacity=".85"/>'
  + '<ellipse cx="18" cy="22.5" rx="8" ry="5.5" fill="#8BAF6A" opacity=".9"/>'
  + '<circle cx="14" cy="15.5" r="3.2" fill="#6B8A50"/><circle cx="22" cy="15.5" r="3.2" fill="#6B8A50"/>'
  + '<circle cx="14" cy="15.5" r="2" fill="#fff" opacity=".9"/><circle cx="22" cy="15.5" r="2" fill="#fff" opacity=".9"/>'
  + '<circle cx="14.5" cy="15.5" r="1" fill="#2A3A1A" style="animation:ld-blink2 4s ease infinite"/>'
  + '<circle cx="22.5" cy="15.5" r="1" fill="#2A3A1A" style="animation:ld-blink2 4s ease infinite"/>'
  + '<path d="M16 22 Q18 24.5 20 22" stroke="#5A7A40" stroke-width="1" fill="none" stroke-linecap="round"/></svg>';
