...
function filterByQuarter(q){if(q==='all')return initiatives.filter(function(i){return i.quarter!=='Backlog';});if(q==='backlog')return initiatives.filter(function(i){return i.quarter==='Backlog';});return initiatives.filter(function(i){return i.quarter===q;});}
function parseNum(v){return parseFloat(String(v).replace(/[^0-9.-]/g,''));}
function fmtDollar(v){if(!v||v==='\u2014')return '\u2014';var n=parseNum(v);if(isNaN(n))return String(v);return '$'+n.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g,',');}
function roiHtml(v){if(!v||v==='\u2014')return '\u2014';var n=parseNum(v);if(isNaN(n))return String(v);var p=Math.round(n*100);return '<span class="'+(p<0?'roi-negative':'roi-positive')+'">'+p+'%</span>';}
...
function roiCalcGroup(items){var a=0,rs=0,rc=0;items.forEach(function(i){var av=parseNum(i.addedValue||'');if(!isNaN(av))a+=av;var r=parseNum(i.roi||'');if(!isNaN(r)){rs+=r;rc++;}});return{count:items.length,av:a,roiAvg:rc?rs/rc:NaN};}
...
function scInitiativesFor(subset,label){var ns=subset.filter(function(i){return i.deliveryStatus==='not-started';}).length,on=subset.filter(function(i){return i.deliveryStatus==='on-track';}).length,ar=subset.filter(function(i){return i.deliveryStatus==='at-risk';}).length,dl=subset.filter(function(i){return i.deliveryStatus==='delayed';}).length;return '<div class="mcard" id="sc-init"><div class="mlabel">Initiatives <span class="mlabel-sub">'+label+'</span></div><div class="mval">'+subset.length+'</div><div class="sc-badges"><span class="sc-badge ds-gray">'+ns+' Not Started</span><span class="sc-badge ds-green">'+on+' On Track</span><span class="sc-badge ds-yellow">'+ar+' At Risk</span><span class="sc-badge ds-red">'+dl+' Delayed</span></div></div>';}
...
function badgeHtml(text,color){var bg=color+'18';return '<span class="badge-inline" style="background:'+bg+';color:'+color+'">'+text+'</span>';}
...
function scGroupedFor(id,label,key,subset,qlabel){var groups={};subset.forEach(function(i){var k=i[key];if(!groups[k])groups[k]={'not-started':0,'on-track':0,'at-risk':0,'delayed':0,count:0};groups[k][i.deliveryStatus]++;groups[k].count++;});var keys=Object.keys(groups);keys.sort();var colorMap=key==='driver'?_driverColorMap:key==='theme'?_themeColorMap:null;function getC(k,ki){if(colorMap&&colorMap[k])return colorMap[k];if(key==='team')return SC_GREENS[ki%SC_GREENS.length];return scGetColor(ki);}var donutSlices=keys.map(function(k,ki){return{v:groups[k].count,c:getC(k,ki)};});var rows=keys.map(function(k,ki){var g=groups[k],pills='';if(g['not-started']>0)pills+='<span class="sc-mini-pill ds-gray">'+g['not-started']+'</span>';if(g['on-track']>0)pills+='<span class="sc-mini-pill ds-green">'+g['on-track']+'</span>';if(g['at-risk']>0)pills+='<span class="sc-mini-pill ds-yellow">'+g['at-risk']+'</span>';if(g['delayed']>0)pills+='<span class="sc-mini-pill ds-red">'+g['delayed']+'</span>';return '<div class="sc-donut-row"><div class="sc-donut-row-left"><span class="sc-donut-dot" style="background:'+getC(k,ki)+'"></span><span class="sc-donut-label">'+k+'</span></div><div class="sc-donut-pills">'+pills+'</div></div>';}).join('');return '<div class="mcard" id="'+id+'"><div class="mlabel">'+label+' <span class="mlabel-sub">'+qlabel+'</span></div><div class="sc-donut-wrap"><div class="sc-donut-chart">'+scDonutSvg(donutSlices)+'</div><div class="sc-donut-rows">'+rows+'</div></div></div>';}
...
function buildQuarterlyProgressBars(){var quarters=['Q1','Q2','Q3','Q4','Backlog'];return '<div class="qp-grid">'+quarters.map(function(q){var items=initiatives.filter(function(i){return i.quarter===q;}),total=items.length||1;var ns=items.filter(function(i){return i.deliveryStatus==='not-started';}).length,on=items.filter(function(i){return i.deliveryStatus==='on-track';}).length,ar=items.filter(function(i){return i.deliveryStatus==='at-risk';}).length,dl=items.filter(function(i){return i.deliveryStatus==='delayed';}).length;var avSum=0;items.forEach(function(i){var n=parseNum(i.addedValue||'');if(!isNaN(n))avSum+=n;});var avLbl=avSum>=1000000?'$'+(avSum/1000000).toFixed(1)+'M':avSum>=1000?'$'+(avSum/1000).toFixed(0)+'K':avSum>0?'$'+avSum.toFixed(0):'\u2014';var nsW=Math.round(ns/total*100),onW=Math.round(on/total*100),arW=Math.round(ar/total*100),dlW=Math.round(dl/total*100);var meta=(ns>0?ns+' not started \u00b7 ':'')+(on>0?on+' on track \u00b7 ':'')+(ar>0?ar+' at risk \u00b7 ':'')+(dl>0?dl+' delayed':'');if(items.length===0)meta='No initiatives';var roiSum=0,roiCount=0;items.forEach(function(i){var r=parseNum(i.roi||'');if(!isNaN(r)){roiSum+=r;roiCount++;}});var roiPct=roiCount?Math.round(roiSum/roiCount*100)+'%':'\u2014';return '<div class="qp-card"><div class="qp-header"><span class="qp-title">'+q+'</span><span class="qp-count">'+items.length+'</span></div><div class="qp-body"><div class="qp-bar-track">'+(nsW>0?'<div style="flex:'+nsW+';background:#6B6B65"></div>':'')+(onW>0?'<div style="flex:'+onW+';background:#3B6D11"></div>':'')+(arW>0?'<div style="flex:'+arW+';background:#CA8A04"></div>':'')+(dlW>0?'<div style="flex:'+dlW+';background:#A93226"></div>':'')+(items.length===0?'<div class="qp-bar-empty"></div>':'')+'</div><div class="qp-meta">'+meta+'</div></div><div class="qp-footer"><div class="qp-roi-label">Avg ROI</div><div class="qp-roi-val">'+roiPct+' <span class="qp-roi-av">('+avLbl+')</span></div></div></div>';}).join('')+'</div>';}
function buildScatterPlot(q){return '<div class="roi-chart-wrap"><div class="roi-chart-label">Added Value vs ROI \u2014 each point is an initiative \u2014 click legend to filter</div><div class="roi-chart-canvas"><canvas id="roiScatterCanvas"></canvas></div></div>';}
function renderScatterChart(q){var subset=q==='all'?initiatives.filter(function(i){return i.quarter!=='Backlog';}):q==='backlog'?initiatives.filter(function(i){return i.quarter==='Backlog';}):initiatives.filter(function(i){return i.quarter===q;});var drivers=[];subset.forEach(function(i){if(drivers.indexOf(i.driver)===-1)drivers.push(i.driver);});drivers.sort();var datasets=drivers.map(function(d){var color=_driverColorMap[d]||'#888780';var pts=subset.filter(function(i){return i.driver===d;}).map(function(i){var av=parseFloat(String(i.addedValue||'').replace(/[^0-9.-]/g,'')),roi=parseFloat(String(i.roi||'').replace(/[^0-9.-]/g,''));if(isNaN(av)||isNaN(roi))return null;return{x:Math.round(roi*100),y:av,label:i.title,techLead:i.techLead};}).filter(Boolean);return{label:d,data:pts,backgroundColor:color+'99',borderColor:color,pointRadius:7,pointHoverRadius:10};});var canvas=document.getElementById('roiScatterCanvas');if(!canvas||!window.Chart)return;if(window._roiChart)window._roiChart.destroy();window._roiChart=new Chart(canvas,{type:'scatter',data:{datasets:datasets},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:true,position:'bottom',labels:{usePointStyle:true,pointStyle:'circle',boxWidth:6,boxHeight:6,padding:16,font:{size:11}}},tooltip:{callbacks:{title:function(items){return items[0].raw.label;},label:function(c){return 'ROI: '+c.raw.x+'%';},afterLabel:function(c){return 'Eng Lead: '+c.raw.techLead;}}}},scales:{x:{title:{display:true,text:'ROI %',font:{size:11}},ticks:{font:{size:11},callback:function(v){return v+'%';}}},y:{title:{display:true,text:'Added Value ($K)',font:{size:11}},ticks:{font:{size:11},callback:function(v){return '$'+v+'K';}}}},layout:{padding:10}}});}
function roiMakeOverallCard(subset,label){var s=roiCalcGroup(subset),p=isNaN(s.roiAvg)?0:Math.round(s.roiAvg*100),c=p<0?'#E24B4A':'#66C220';return '<div class="roi-overall-card"><div class="roi-card-eyebrow">Overall ROI &mdash; '+label+'</div><div><div class="roi-card-big">'+s.count+'</div><div class="roi-card-sub">initiatives</div></div><div class="roi-divider"></div><div><div class="roi-card-big-20">'+roiFmtAV(s.av)+'</div><div class="roi-card-sub">added value</div></div><div class="roi-divider"></div><div><div class="roi-card-big-20" style="color:'+c+'">'+p+'%</div><div class="roi-card-sub">avg ROI</div></div></div>';}
function roiMakeBarCard(subset,title,label,key){var keys=[];subset.forEach(function(i){if(keys.indexOf(i[key])===-1)keys.push(i[key]);});keys.sort();var colorMap=key==='driver'?_driverColorMap:key==='theme'?_themeColorMap:null;var rows=keys.map(function(k,ki){var s=roiCalcGroup(subset.filter(function(i){return i[key]===k;})),p=isNaN(s.roiAvg)?0:Math.round(s.roiAvg*100),tc=p<0?'#E24B4A':'#3B6D11',isLast=ki===keys.length-1;var kHtml=colorMap?badgeHtml(k,colorMap[k]||'#888780'):key==='team'?badgeHtml(k,SC_GREENS[ki%SC_GREENS.length]):'<span style="font-size:12px;color:var(--text)">'+k+'</span>';return '<div class="roi-bar-row'+(isLast?'':'')+'"'+(isLast?'':' style="border-bottom:0.5px solid var(--border)"')+'><div>'+kHtml+'</div><div class="roi-bar-val">'+roiFmtEU(s.av)+'</div><div class="roi-bar-pct" style="color:'+tc+'">'+p+'%</div></div>';}).join('');var hdr='<div class="roi-bar-header"><div></div><div class="roi-bar-header-label">Added Value</div><div class="roi-bar-header-label roi-bar-header-label-sm">Avg ROI</div></div>';return '<div class="roi-bar-card"><div class="roi-card-eyebrow" style="margin-bottom:14px">'+title+' &mdash; '+label+'</div>'+hdr+rows+'</div>';}
...
function roiRenderContent(q){var label=q==='all'?'All Year':q==='backlog'?'Backlog':q,subset=q==='all'?initiatives.filter(function(i){return i.quarter!=='Backlog';}):q==='backlog'?initiatives.filter(function(i){return i.quarter==='Backlog';}):initiatives.filter(function(i){return i.quarter===q;});return '<div class="roi-grid">'+roiMakeOverallCard(subset,label)+roiMakeBarCard(subset,'ROI by Driver',label,'driver')+roiMakeBarCard(subset,'ROI by Theme',label,'theme')+roiMakeBarCard(subset,'ROI by Team',label,'team')+'</div>'+roiMakeTable(subset);}
...
function buildGantt() {
  ...
  var toggle = '<div class="gantt-toggle">'
    + '<span class="gantt-toggle-label">Group by</span>'
    + ['driver','theme','team'].map(function(v) {
      var lbl = v === 'team' ? 'Team' : v === 'theme' ? 'Theme' : 'Driver';
      var act = v === groupKey;
      return '<button class="gantt-toggle-btn'+(act?' act':'')+'" data-ganttgroup="' + v + '">' + lbl + '</button>';
    }).join('')
    + '</div>';
  ...
  var thead = '<thead><tr>'
    ...
    + qHeaders.map(function(q) {
      var isCurrent = q === cq;
      return '<th class="gantt-th-q'+(isCurrent?' current':'')+'">' + q + '</th>';
    }).join('')
    + '</tr></thead>';
  ...
  groupNames.forEach(function(gName) {
    ...
    groups[gName].forEach(function(i) {
      ...
      var nameCell = '<td class="gantt-name-cell">'
        ...
        + '<div class="gantt-name-meta">'
        + sl[0] + ': <span class="gantt-name-meta-val">' + v1 + '</span>'
        + ' \u00b7 ' + sl[1] + ': <span class="gantt-name-meta-val">' + v2 + '</span>'
        + '</div></td>';

      var qCells = qHeaders.map(function(q) {
        var isCurrent = q === cq;
        var active = qs.indexOf(q) > -1;
        if (!active) return '<td class="gantt-q-cell'+(isCurrent?' current':'')+'"></td>';
        var bid = 'gbar-' + barIdx++;
          ...
          return '<td class="gantt-q-cell'+(isCurrent?' current':'')+'">'
