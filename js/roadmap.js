...
function roiRenderContent(q){var label=q==='all'?'All Year':q==='backlog'?'Backlog':q,subset=q==='all'?initiatives.filter(function(i){return i.quarter!=='Backlog';}):q==='backlog'?initiatives.filter(function(i){return i.quarter==='Backlog';}):initiatives.filter(function(i){return i.quarter===q;});return '<div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px;margin-bottom:16px">'+roiMakeOverallCard(subset,label)+roiMakeBarCard(subset,'ROI by Driver',label,'driver')+roiMakeBarCard(subset,'ROI by Theme',label,'theme')+roiMakeBarCard(subset,'ROI by Team',label,'team')+'</div>'+roiMakeTable(subset);}
