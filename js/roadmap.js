...
var SC_PALETTE=['#66C220','#378ADD','#BA7517','#D4537E','#1D9E75','#7F77DD','#D85A30','#3B6D11','#993556','#534AB7','#E24B4A','#0F6E56'];
function scGetColor(idx){return SC_PALETTE[idx%SC_PALETTE.length];}
var _driverColorMap={}, _themeColorMap={};
var SC_BLUES=['#378ADD','#185FA5','#85B7EB','#0C447C','#5A9BD5','#2B72C2','#6FA8E0','#1049A0','#4B8FD0','#042C53','#B5D4F4','#3266AD'];
function buildColorMaps(){var ds=[],ts=[];initiatives.forEach(function(i){if(ds.indexOf(i.driver)===-1)ds.push(i.driver);if(ts.indexOf(i.theme)===-1)ts.push(i.theme);});ds.sort();ts.sort();_driverColorMap={};_themeColorMap={};ds.forEach(function(d,i){_driverColorMap[d]=scGetColor(i);});ts.forEach(function(t,i){_themeColorMap[t]=SC_BLUES[i%SC_BLUES.length];});}
...
function roiMakeBarCard(subset,title,label,key){var keys=[];subset.forEach(function(i){if(keys.indexOf(i[key])===-1)keys.push(i[key]);});keys.sort();var colorMap=key==='driver'?_driverColorMap:key==='theme'?_themeColorMap:null;var rows=keys.map(function(k,ki){var s=roiCalcGroup(subset.filter(function(i){return i[key]===k;})),p=isNaN(s.roiAvg)?0:Math.round(s.roiAvg*100),tc=p<0?'#E24B4A':'#3B6D11',isLast=ki===keys.length-1;var kHtml=colorMap?badgeHtml(k,colorMap[k]||'#888780'):key==='team'?badgeHtml(k,SC_BLUES[ki%SC_BLUES.length]):'<span style="font-size:12px;color:var(--text)">'+k+'</span>';return '<div style="display:grid;grid-template-columns:1fr auto auto;gap:8px;padding:10px 0;'+(isLast?'':' border-bottom:0.5px solid var(--border);')+'align-items:center"><div>'+kHtml+'</div><div style="font-size:12px;font-weight:500;text-align:right;min-width:80px">'+roiFmtEU(s.av)+'</div><div style="font-size:12px;font-weight:500;color:'+tc+';text-align:right;min-width:48px">'+p+'%</div></div>';}).join('');var hdr='<div style="display:grid;grid-template-columns:1fr auto auto;gap:8px;padding:8px 0;border-bottom:0.5px solid var(--border)"><div></div><div style="font-size:10px;font-weight:500;text-transform:uppercase;letter-spacing:.4px;color:var(--faint);text-align:right;min-width:80px">Added Value</div><div style="font-size:10px;font-weight:500;text-transform:uppercase;letter-spacing:.4px;color:var(--faint);text-align:right;min-width:48px">Avg ROI</div></div>';return '<div style="background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:16px"><div style="font-size:10px;font-weight:500;text-transform:uppercase;letter-spacing:.5px;color:var(--faint);margin-bottom:14px">'+title+' &mdash; '+label+'</div>'+hdr+rows+'</div>';}
